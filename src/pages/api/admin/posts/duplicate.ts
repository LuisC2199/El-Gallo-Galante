// ---------------------------------------------------------------------------
// POST /api/admin/posts/duplicate – duplicate an existing post
// ---------------------------------------------------------------------------
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    // Serialize & create
    const fileContent = serializePost(fm as Record<string, unknown>, mdBody);
    const filePath = `${POSTS_DIR}/${newSlug}.md`;

    const result = await createTextFile(
      cfg,
      filePath,
      fileContent,
      `admin: duplicate post ${body.slug} → ${newSlug}`,
    );
